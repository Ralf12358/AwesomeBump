...
void OpenGLImageEditor::render()
{
    ...
    QOpenGLFramebufferObject *activeFBO = activeImage->fbo;
    ...
    if((activeImage->bSkipProcessing) && (activeImage->imageType != MATERIAL_TEXTURE))
    ...
    if(!bSkipProcessing == true)
    {
        ...
        GLCHK( glBindTexture(GL_TEXTURE_2D, targetImageMaterial->scr_tex_id->textureId()) );
        GLCHK( glActiveTexture(GL_TEXTURE0) );

        //if(int(activeImage->currentMaterialIndeks) < 0)
        //{
        copyTex2FBO(activeImage->scr_tex_id->textureId(),activeImage->fbo);
        //}
        ...
        case(NORMAL_TEXTURE):
        {
            ...
            case(INPUT_FROM_HEIGHT_INPUT):
                ...
                if(conversionType == CONVERT_NONE)
                {
                    ...
                    copyTex2FBO(targetImageHeight->scr_tex_id->textureId(),activeFBO);
            ...
            case(INPUT_FROM_HEIGHT_OUTPUT):
                ...
                if(!targetImageHeight->bSkipProcessing)
                    bTransformUVs = false;
                break;
            default:
                break;
        ...
        case(SPECULAR_TEXTURE):
        {
            ...
            case(INPUT_FROM_DIFFUSE_INPUT):
                copyTex2FBO(targetImageDiffuse->scr_tex_id->textureId(), activeFBO);
                break;
            case(INPUT_FROM_DIFFUSE_OUTPUT):
                copyFBO(targetImageDiffuse->fbo,activeFBO);
                if(!targetImageDiffuse->bSkipProcessing)
                    bTransformUVs = false;
                break;
            default:
                break;
        ...
        case(OCCLUSION_TEXTURE):
        {
            ...
            case(INPUT_FROM_OCCLUSION_INPUT):
                if(conversionType == CONVERT_FROM_HN_TO_OC)
                {
                    ...
                    bSkipStandardProcessing =  true;
                    if(!targetImageHeight->bSkipProcessing && !targetImageNormal->bSkipProcessing)
                        bTransformUVs = false;
            ...
            case(INPUT_FROM_HI_NI):
                // Ambient occlusion is calculated from normal and height map.
                // Skip some parts of the processing.
                applyOcclusionFilter(targetImageHeight->scr_tex_id->textureId(), targetImageNormal->scr_tex_id->textureId(), activeFBO);
                break;
        ...
        case(HEIGHT_TEXTURE):
        {
            if(conversionType == CONVERT_FROM_N_TO_H)
            {
                ...
                if(!targetImageNormal->bSkipProcessing)
                    bTransformUVs = false;
            }
            break;
        ...
        if(!bSkipStandardProcessing)
        {
            ...
            if(activeImage->properties->EnableRemoveShading)
            {
                ...
                applyRemoveShadingFilter(auxFBO2, targetImageOcclusion->fbo, activeFBO, auxFBO1);
                copyFBO(auxFBO1,activeFBO);
            }
        ...
        case(CONVERT_FROM_D_TO_O):
            ...
            applyOcclusionFilter(targetImageHeight->scr_tex_id->textureId(), targetImageNormal->scr_tex_id->textureId(), targetImageOcclusion->fbo);
            targetImageOcclusion->updateSrcTexId(targetImageOcclusion->fbo);
            copyTex2FBO(activeImage->scr_tex_id->textureId(), targetImageSpecular->fbo);
            targetImageSpecular->updateSrcTexId(targetImageSpecular->fbo);
            copyTex2FBO(activeImage->scr_tex_id->textureId(), targetImageRoughness->fbo);
            targetImageRoughness->updateSrcTexId(targetImageRoughness->fbo);
            copyTex2FBO(activeImage->scr_tex_id->textureId(), targetImageMetallic->fbo);
...
void OpenGLImageEditor::resizeGL(int width, int height)
{
    ...
    if (isValid())
    {
        ...
        else
        {
            ...
            else if (!activeImage->fbo)
                qWarning() << "  activeImage->fbo is null";
        }
    }
    else
    {
...
void OpenGLImageEditor::setActiveImage(OpenGLFramebufferObjectProperties *ptr)
{
    activeImage = ptr;
    update();
}
...
{
    OpenGLFramebufferObjectProperties::seamlessMode = mode;
    update();
}
                                                          ...
                                                          QOpenGLFramebufferObject *outputFBO)
{
    ...
    if(OpenGLFramebufferObjectProperties::currentMaterialIndeks != MATERIALS_DISABLED)
    {
                                                  ...
                                                  QOpenGLFramebufferObject *outputFBO)
{
    ...
    switch(OpenGLFramebufferObjectProperties::seamlessContrastInputType)
    {
    default:
    case(INPUT_FROM_HEIGHT_INPUT):
        //copyFBO(targetImageHeight->ref_fbo,activeImage->aux2_fbo);
        copyTex2FBO(targetImageHeight->scr_tex_id->textureId(), auxFBO2);
        break;
    case(INPUT_FROM_DIFFUSE_INPUT):
        //copyFBO(targetImageDiffuse->ref_fbo,activeImage->aux2_fbo);
        copyTex2FBO(targetImageDiffuse->scr_tex_id->textureId(), auxFBO2);
        break;
    };

    // When translations are applied, first one has to translate.
    // else the contrast mask image
    if(OpenGLFramebufferObjectProperties::bSeamlessTranslationsFirst)
    {
    ...
    GLCHK( program->setUniformValue("quad_scale", QVector2D(1.0,1.0)) );
    GLCHK( program->setUniformValue("quad_pos", QVector2D(0.0,0.0)) );
    GLCHK( program->setUniformValue("make_seamless_radius"           , OpenGLFramebufferObjectProperties::seamlessSimpleModeRadius) );
    GLCHK( program->setUniformValue("gui_seamless_contrast_strenght" , OpenGLFramebufferObjectProperties::seamlessContrastStrenght) );
    GLCHK( program->setUniformValue("gui_seamless_contrast_power"    , OpenGLFramebufferObjectProperties::seamlessContrastPower) );

    GLCHK( glViewport(0,0,inputFBO->width(),inputFBO->height()) );
    switch(OpenGLFramebufferObjectProperties::seamlessSimpleModeDirection)
    {
    default:
    case(0): //XY
                                            ...
                                            QOpenGLFramebufferObject *outputFBO)
{
    ...
    default:
    case(INPUT_FROM_HEIGHT_INPUT):
        copyTex2FBO(targetImageHeight->scr_tex_id->textureId(), auxFBO1);
        break;
    case(INPUT_FROM_DIFFUSE_INPUT):
        copyTex2FBO(targetImageDiffuse->scr_tex_id->textureId(), auxFBO1);
        break;
    };
    ...
    GLCHK( program->setUniformValue("gui_seamless_contrast_strenght", OpenGLFramebufferObjectProperties::seamlessContrastStrenght) );
    GLCHK( program->setUniformValue("gui_seamless_contrast_power", OpenGLFramebufferObjectProperties::seamlessContrastPower) );
    GLCHK( program->setUniformValue("gui_seamless_mode", (int)OpenGLFramebufferObjectProperties::seamlessMode) );
    GLCHK( program->setUniformValue("gui_seamless_mirror_type", OpenGLFramebufferObjectProperties::seamlessMirroModeType) );

    // Send random angles.
    QMatrix3x3 random_angles;
    for(int i = 0; i < 9; i++)
        random_angles.data()[i] = OpenGLFramebufferObjectProperties::seamlessRandomTiling.angles[i];

    GLCHK( program->setUniformValue("gui_seamless_random_angles" , random_angles) );
    GLCHK( program->setUniformValue("gui_seamless_random_phase" , OpenGLFramebufferObjectProperties::seamlessRandomTiling.common_phase) );
    GLCHK( program->setUniformValue("gui_seamless_random_inner_radius" , OpenGLFramebufferObjectProperties::seamlessRandomTiling.inner_radius) );
    GLCHK( program->setUniformValue("gui_seamless_random_outer_radius" , OpenGLFramebufferObjectProperties::seamlessRandomTiling.outer_radius) );
                                               ...
                                               QOpenGLFramebufferObject *outputFBO)
{
    ...
    GLCHK( glActiveTexture(GL_TEXTURE1) );
    GLCHK( glBindTexture(GL_TEXTURE_2D, activeImage->normalMixerInputTexId->textureId()) );
...
void OpenGLImageEditor::applyNormalToHeight(OpenGLFramebufferObjectProperties *image,
                                                    ...
                                                    QOpenGLFramebufferObject *outputFBO)
{
    ...
    if(OpenGLFramebufferObjectProperties::currentMaterialIndeks != MATERIALS_DISABLED)
    {
        ...
        int currentMaterialIndex = OpenGLFramebufferObjectProperties::currentMaterialIndeks;
...
void OpenGLImageEditor::applyAllUVsTransforms(QOpenGLFramebufferObject *inoutFBO)
{
    ...
    // Make seamless.
    switch(OpenGLFramebufferObjectProperties::seamlessMode)
    {
                                                    ...
                                                    QOpenGLFramebufferObject *outputFBO)
{
    ...
    GLCHK( glBindTexture(GL_TEXTURE_2D, inputFBO->texture()) );
    GLCHK( glActiveTexture(GL_TEXTURE1) );
    GLCHK( glBindTexture(GL_TEXTURE_2D, targetImageNormal->scr_tex_id->textureId()) );
    GLCHK( glDrawElements(GL_TRIANGLES, 3*2, GL_UNSIGNED_INT, 0) );
    GLCHK( glActiveTexture(GL_TEXTURE0) );
    outputFBO->bindDefault();
}
...
void OpenGLImageEditor::relativeMouseMoveEvent(int dx, int dy, bool* wrapMouse, Qt::MouseButtons buttons)
{
    ...
    if(activeImage->imageType == METALLIC_TEXTURE &&
            buttons & Qt::LeftButton)
    {
        QMessageBox msgBox;
        msgBox.setText("Warning!");
    ...
    if(activeImage->imageType == SPECULAR_TEXTURE &&
            buttons & Qt::LeftButton)
    {
        QMessageBox msgBox;
        msgBox.setText("Warning!");
