...
void OpenGLImageEditor::applySeamlessFilter(QOpenGLFramebufferObject* inputFBO,
    ...
    for(int i = 0; i < 9; i++)random_angles.data()[i] = OpenGLFramebufferObjectProperties::seamlessRandomTiling.angles[i];
...
void OpenGLImageEditor::applyNormalToHeight(OpenGLFramebufferObjectProperties* image,QOpenGLFramebufferObject* normalFBO,
...
void OpenGLImageEditor::copyFBO(QOpenGLFramebufferObject* src,QOpenGLFramebufferObject* dst){
#ifdef USE_OPENGL_330
    program = filter_programs["mode_normal_filter"];
    program->bind();
    updateProgramUniforms(0);
...
void OpenGLImageEditor::copyTex2FBO(GLuint src_tex_id,QOpenGLFramebufferObject* dst){
...
void OpenGLImageEditor::relativeMouseMoveEvent(int dx, int dy, bool* wrapMouse, Qt::MouseButtons buttons)
{
    ...
    if(activeImage->imageType != GRUNGE_TEXTURE)
    if(OpenGLFramebufferObjectProperties::currentMaterialIndeks != MATERIALS_DISABLED && buttons & Qt::LeftButton){
        QMessageBox msgBox;
