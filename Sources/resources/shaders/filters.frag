...
vec4 ffilter(){
    ...
    if(uv_scaling_mode == 1)
	{
        ...
        // Calculating proper mapping for UV coordinates.
    ...
    if(gui_perspective_mode == 0)
	{
	    ...
	    transf_pos = p1*corner1 + p2*corner2 + p3*corner3 + p4*corner4;
...
#endif
	// Normalise to 1.
    float remove_sh = gui_remove_shading / 100.0;
    // Colour.
	vec4 colorA   = texture(layerA, v2QuadCoords.xy);
	// ao.
	vec4 colorB   = texture(layerB, v2QuadCoords.xy);
    // Refcolour.
	vec4 refColor = texture(layerC, v2QuadCoords.xy);
...
vec4 ffilter(){
	...
	else
	{
		...
		// Blured image.
		vec4 scolor = texture(layerC, v2QuadCoords.xy);
        // Draw mask on image.
		...
		{
			scolor  = mix(ocolor, scolor, mask * gui_gauss_blending);
		}
        return scolor;
    }
}
...
float sobel_edge_detection()
{
    ...
    // Using unrolled loop and macro function,
	// because the offset parameter of the textureOffset function must be a constant or literal value.
...
// 1 - simple, 2 - mirror
uniform int gui_seamless_mode;
// 0 - xy, 1 - x , 2 - y mirror
uniform int gui_seamless_mirror_type;
...
vec4 ffilter(){
    ...
    if(gui_seamless_mode == 0)
	{
        ...
        vec4 colorB = texture(layerA, vec2(tc.x, tc.y) + offset);
...
// Special treatment for basemap conversion.
...
/**
 * Takes two colors RBG and calculates the similarity between them.
 * @return A number which reflects the similarity.
 */
float color_dist(vec3 colorA, vec3 colorB)
{
    // If 1.0 the results should be same like without any modifications.
    if(gui_gray_scale_range_tol == 0.0) return 1.0;

    // Distance between the two colors.
	float cdist = length(colorA - colorB);
    float tol   = gui_gray_scale_range_tol / 5.0;
    // Example weight calculation, but it can be any.
	return 1 - exp(-tol * cdist);
	// Another example weight calculation.
    // return clamp(cdist * gui_gray_scale_range_tol / 10.0, 0, 1);
}
...
vec4 ffilter(){
    ...
    // Additional filtering if base map conversion is enabled.
	// If the distance between two colour vectors is close to zero they are considered near.
    // Force colors which are near to be 0 (for min color) or 1 (for max color),
	// where min and max values are selected by the user.
...
// Adding random number to height image to remove aliasing problem, resulting from numerics.
...
vec4 ffilter(){
	...
	// Rotate.
	newTexCoords = vec2(newTexCoords.x * cos(gui_normal_mixer_angle) + newTexCoords.y * sin(gui_normal_mixer_angle),
			-newTexCoords.x * sin(gui_normal_mixer_angle) + newTexCoords.y * cos(gui_normal_mixer_angle));
	// Scale coordinates.
	...
	// Rotate normals.
	normalB = vec3(normalB.x * cos(-gui_normal_mixer_angle)+ normalB.y * sin(-gui_normal_mixer_angle),
			-normalB.x * sin(-gui_normal_mixer_angle) + normalB.y * cos(-gui_normal_mixer_angle),
			normalB.z);

	vec3 finalNormal = vec3(normalA.xy, normalA.z) + vec3(normalB.xy * ndepth,0);
    ...
    if(gui_combine_normals == 0)
	{
		for(int i = -radius ; i <= radius ; i++)
		{
			for(int j = -radius ; j <= radius ; j++)
			{
		        ...
		        float w = mix(length(normal.xy), 1 / (20 * gaussian(vec2(i, j),
						gui_filter_radius) * length(normal.xy) + 1),
...
float g_intensity = gui_ssao_intensity;
float g_scale = 5 * gui_ssao_depth;
// Controls the width of the occlusion cone.
float g_bias = 0.05 * gui_ssao_bias;
...
vec4 ffilter(){
#endif
	vec3 p = getPosition(v2QuadCoords.xy);
	vec3 n = getNormal(v2QuadCoords.xy);
	float ao = 0.0f;
...
vec4 height_clamp2(vec4 inputc, float weight,float vmin,float vmax)
{
    ...
    if(inputc.r > vmax)
		outputValue.r = vmax + dmax_ave.r;
    if(inputc.g > vmax)
		outputValue.g = vmax + dmax_ave.g;
    if(inputc.b > vmax)
		outputValue.b = vmax + dmax_ave.b;

    vec4 dmin_ave = weight * (inputc - vec4(vmin));

    if(inputc.r < vmin)
		outputValue.r = vmin + dmin_ave.r;
    if(inputc.g < vmin)
		outputValue.g = vmin + dmin_ave.g;
    if(inputc.b < vmin)
		outputValue.b = vmin + dmin_ave.b;
...
vec4 ffilter(){
    ...
    // "details" slider in gui
	float w = gui_height_proc_ave_radius / 50.0;
...
vec4 ffilter(){
#endif
    // Different approaches to measure difference in colors.
    float angle;
    float amp = (1 + 5 * gui_roughness_color_amplifier);
    float offset = gui_roughness_color_offset * 2;

    if(gui_roughness_color_method == 1)
	{
		// Angle based A.
        // Treat colors like normals
        vec3 colorA = normalize(texture(layerA, v2QuadCoords.xy).rgb - 0.5);
        // Normalised value of picked color.
		vec3 maskColor = normalize(gui_roughness_picked_color - 0.5);
        angle = clamp(dot(colorA, maskColor) * amp + offset, 0.0, 1.0);
    }
	else if(gui_roughness_color_method == 2)
	{
		// Angle based B.
        vec3 colorA = normalize(texture(layerA, v2QuadCoords.xy).rgb);
        // Normalised value of picked color.
        vec3 maskColor = normalize(gui_roughness_picked_color);
        angle = clamp(dot(colorA, maskColor) * amp + offset, 0.0, 1.0);
    }
	else if(gui_roughness_color_method == 3)
	{
		// Distance based.
...
{   
    ...
    int materialIndex = int(255 * 255 * 255 * materialColor.r) +
						int(255 * 255 * materialColor.g) +
						int(255 * materialColor.b);

    if(material_id >= 0)
	{
		// If current ID is different than -1
        if(materialIndex == material_id)
		{
			// Compare colors and process only the mask region.
	...
	{
		// Only draw the last image.
        FragColor = texture(layerA, v2QuadCoords.xy);
    }
	else
	{
		// Normal processing (materials disabled).
            FragColor   = filterMode();
    }
 }
